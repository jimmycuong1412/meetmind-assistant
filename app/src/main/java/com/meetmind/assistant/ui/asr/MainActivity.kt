...
import com.meetmind.assistant.ui.ui.theme.LibellulaTheme
...
import com.meetmind.assistant.ui.screens.*
import com.meetmind.assistant.presentation.setup.SetupViewModel
...
fun LibellulaApp(quickStart: Boolean = false) {
    ...
    NavHost(
        ...
        composable("sessions") {
            SessionsScreen(
                ...
                onNavigateToSessionDetailsForAnalyze = { sessionId ->
                    // AI sparkle button: deep-link to session details with auto-trigger flag
                    // so the working confirm dialog opens immediately on arrival.
                    navController.navigate("session_details/$sessionId?autoAnalyze=true")
                },
        ...
        // Session details screen — optional query params for search deep-linking, initial tab,
        // and auto-triggering history insight generation when arrived from the sessions list AI button.
        composable(
            route = "session_details/{sessionId}?highlightId={highlightId}&initialTab={initialTab}&autoAnalyze={autoAnalyze}&autoAnalyzeOutputLanguage={autoAnalyzeOutputLanguage}",
            arguments = listOf(
                ...
                navArgument("autoAnalyzeOutputLanguage") {
                    type = NavType.StringType
                    nullable = true
                    defaultValue = null
                }
            ...
            SessionDetailsScreen(
                ...
                onNavigateToSession = { newSessionId ->
                    ...
                    navController.navigate("session_details/$newSessionId?initialTab=1") {
                        popUpTo("session_details/{sessionId}?highlightId={highlightId}&initialTab={initialTab}&autoAnalyze={autoAnalyze}&autoAnalyzeOutputLanguage={autoAnalyzeOutputLanguage}") {
                            inclusive = true
                        }
                    }
                }
