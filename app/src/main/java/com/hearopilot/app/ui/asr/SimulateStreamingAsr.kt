...
object SimulateStreamingAsr {
    ...
    fun initVad(
        ...
        config.sileroVadModelConfig.minSilenceDuration = minSilenceDuration
        config.sileroVadModelConfig.maxSpeechDuration = maxSpeechDuration
        config.sileroVadModelConfig.threshold = threshold

        _vad = Vad(
            assetManager = assetManager,
            config = config,
        )
        Log.i(TAG, "sherpa-onnx vad initialized with custom params: " +
                "minSilence=${minSilenceDuration}s, maxSpeech=${maxSpeechDuration}s, threshold=$threshold")
    }
}
